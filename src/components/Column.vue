...
<template>
 <div class="bg-grey-darken-3 ma-2 mb-0 rounded d-flex flex-column column">
  <div class="d-flex justify-space-between align-center px-4 pt-2">
   ...
   <v-btn density="compact" icon="mdi-plus" variant="text" @click="emit('removeList')">
    <IconDelete />
    <v-tooltip content-class="bg-grey-darken-2" activator="parent" location="top">Delete list</v-tooltip>
   </v-btn>
  </div>
  <div class="pa-3 flex-grow-1 overflow-y-auto column-container">
   <slot />
  </div>
  <v-card class="mx-auto mt-2" width="100%" variant="plain" hover>
   <v-toolbar v-if="!isCreatingNewTask" density="compact" class="bg-grey-darken-3 px-4" @click="toggle">
    <IconPlus />
